import { NextResponse } from "next/server";
import { getServerSession } from "next-auth";
import { revalidatePath, revalidateTag } from "next/cache";
import { authOptions } from "@/lib/auth";
import { prisma } from "@/lib/prisma";
import { sanitizePlainText } from "@/lib/sanitize";
import { SITE_CONTENT_TAG, ABOUT_KEY, type AboutContent } from "@/lib/site-content";
import { ICON_OPTIONS, type IconCardContent } from "@/lib/site-content-defaults";

const MAX_TEXT = 300;
const MAX_ITEMS = 8;

const VALID_ICON_KEYS = new Set<string>(ICON_OPTIONS.map((opt) => opt.key));

function cleanText(value: unknown, maxLength = MAX_TEXT): string {
  return sanitizePlainText(typeof value === "string" ? value : "").slice(0, maxLength);
}

function cleanIconCard(raw: unknown): IconCardContent | null {
  if (!raw || typeof raw !== "object") return null;
  const r = raw as Record<string, unknown>;
  const title = cleanText(r.title, 150);
  const description = cleanText(r.description, 400);
  if (!title || !description) return null;
  const icon = typeof r.icon === "string" && VALID_ICON_KEYS.has(r.icon) ? r.icon : ICON_OPTIONS[0].key;
  return { title, description, icon: icon as IconCardContent["icon"] };
}

export async function POST(request: Request) {
  const session = await getServerSession(authOptions);
  if (!session?.user || session.user.role !== "ADMIN") {
    return NextResponse.json({ error: "دسترسی غیرمجاز است." }, { status: 401 });
  }

  const body = await request.json().catch(() => null);
  if (!body || typeof body !== "object") {
    return NextResponse.json({ error: "داده ارسالی نامعتبر است." }, { status: 400 });
  }

  const principles = Array.isArray(body.principles)
    ? body.principles
        .slice(0, MAX_ITEMS)
        .map(cleanIconCard)
        .filter((item: IconCardContent | null): item is IconCardContent => item !== null)
    : [];

  const yearsValue = typeof body.yearsValue === "number" && Number.isFinite(body.yearsValue) ? Math.trunc(body.yearsValue) : 0;

  const content: AboutContent = {
    title: cleanText(body.title, 200),
    body: cleanText(body.body, 2000),
    principles,
    yearsValue,
    registrationNumber: cleanText(body.registrationNumber, 60),
    registrationLabel: cleanText(body.registrationLabel, 100),
    trustBadgeTitle: cleanText(body.trustBadgeTitle, 150),
    trustBadgeText: cleanText(body.trustBadgeText, 500),
  };

  await prisma.siteContent.upsert({
    where: { key: ABOUT_KEY },
    update: { value: JSON.stringify(content) },
    create: { key: ABOUT_KEY, value: JSON.stringify(content) },
  });

  revalidateTag(SITE_CONTENT_TAG);
  revalidatePath("/about");

  return NextResponse.json({ ok: true, content });
}
